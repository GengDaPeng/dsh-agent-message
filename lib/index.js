 ...
 * - check_delivery：按需回执（delivered/claimed/discarded/unknown + targetStatus），默认零播报。
 */
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const source = form === 'relay'
        ...
        : { kind: 'user', plugin: 'dsh-agent-message', form: 'user', senderSessionId: String(me.id), senderTitle: myTitle }
      const message = makeMessage('From Agent · ' + myTitle + ': @' + String(me.id) + '\n' + args.content, source)
