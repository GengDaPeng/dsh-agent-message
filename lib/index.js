/**
 * dsh-agent-message — host half (agent preset).
 *
 * Registers two model-facing tools for cross-session messaging between live
 * agents. Relay messages render as ordinary user bubbles and carry the sender's
 * session id in a compact footer so the receiving agent can reply. `agents` is
 * the host-plane registry; every capability here is model-facing and per-session.
 */
