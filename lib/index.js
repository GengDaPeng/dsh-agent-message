...
export function apply(ctx, config) {
  ...
  function rememberSent(messageId, to, mode) {
    sent.set(messageId, { messageId, to, at: Date.now(), mode })
  ...
  /** 回执状态：delivered 排队中 / claimed 已读 / processing 处理中 / discarded 被丢弃 / unknown 查无此消息。 */
  async function deliveryStateOf(entry) {
    ...
    if (target !== undefined) {
      ...
      if (state.claimed.has(messageId)) return target.status === 'running' ? 'processing' : 'claimed'
      return 'unknown'
    }
    const persistence = ctx.get('sessionPersistence')
    ...
    const state = foldInbox(events.slice(meta.seedLength ?? 0))
    const pending = state['next-turn'].concat(state['next-step'])
    if (pending.some((m) => m.id === messageId)) return 'delivered'
    if (state.discarded.has(messageId)) return 'discarded'
  ...
  ctx.on('agent/session-start', ({ agent }) => {
    if (agent !== undefined && agent.inbox !== undefined && agent.inbox.hasPending) {
      agent.wakeDriver()
    }
  })
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const source = form === 'relay'
        ? { kind: 'plugin', plugin: 'dsh-agent-message', form: 'relay', senderSessionId: String(me.id), senderTitle: myTitle }
        : { kind: 'user', senderSessionId: String(me.id), senderTitle: myTitle }
      const message = makeMessage('来自 Agent「' + myTitle + '」：\n' + args.content, source)
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      '状态：delivered=已送达·排队中；claimed=对方已读（被认领）；processing=对方正在处理；discarded=被丢弃；unknown=查无此消息。' +
