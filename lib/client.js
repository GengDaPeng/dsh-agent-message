  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function senderTitle(element) {
        ...
        const current = text.match(/^From Agent(?: ·)? (.+):$/);
        const previous = text.match(/^来自 Agent · (.+)$/);
        const legacy = text.match(/^来自 Agent「(.+)」\s*[·：:]?$/);
        const title = (current || previous || legacy)?.[1]?.trim() || "";
        if (title) {
          label.textContent = "";
      ...
      const css =
        ...
        ".agent-msg-session-link { display: block; width: fit-content; padding: 0; background: transparent; color: inherit; cursor: pointer; font-size: 13px; font-weight: 600; line-height: 1.5; text-decoration: none; } " +
